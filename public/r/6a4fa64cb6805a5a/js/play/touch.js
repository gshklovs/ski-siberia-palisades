...
//   right half  hold              jumpHeld + grab     levels; §4.4 writes both
//   right half  hold, airborne    GRAB                specs/0057 §5.1 — and the
//                                 stick picks it      stick's own move keys name
//                                                     it (W/A/S/D, D12)
...
function start() {
  ...
  // specs/0055 §4/§5 (W7, Greg 2026-09-06) — cell A's arc, transposed. The cell
  // mocks the stick at r 52 and this build draws it at r 58 (P5: "the joystick
  // ring unchanged in size", and the cell's own note says radius, anchor and
  // dead zone are all unchanged from this file). So the two PIXEL-SCALE numbers
  // are the cell's literal ones — a 4 px band, 1 px inside the ring's edge —
  // and the one number that is a fraction of the ring, the nub, scales with it:
  // 14/104 of the cell's box is 15.6 of this one, taken as 16 so the centring
  // margin stays a whole pixel.
  const ARC_W = 4;            // px — the band's thickness (cell: 47→51 of 52)
  const ARC_DEG = 80;         // ° — how wide the wedge is (cell: 0→80deg)
  const NUB = 16;             // px — the thumb (cell: 14 on a 104 box)
  const DEAD = 0.06;          // below this deflection there is no direction to
                              // draw, so the arc and the vector stay off
...
/* THE ARC — the held direction, drawn on the ring itself, the same arc idiom as
   the dial. A conic wedge masked to a band: ${ARC_W} px thick, sitting 1 px
   inside the ring's outer edge (the cell's band is 47→51 px in a 52 px radius,
   which is 4 px thick and 1 px in), spanning ${ARC_DEG}° centred on the push.
...
.ptouch__arc {
  ...
  margin: ${-RING}px 0 0 ${-RING}px; border-radius: 50%;
  opacity: 0;
  background: conic-gradient(from calc(var(--pa) - ${ARC_DEG / 2}deg),
    var(--p-grad-to) 0 ${ARC_DEG}deg, transparent ${ARC_DEG}deg);
  -webkit-mask: radial-gradient(circle at 50% 50%,
    transparent 0 ${RING - 1 - ARC_W}px, #000 ${RING - 1 - ARC_W}px ${RING - 1}px, transparent ${RING - 1}px);
...
/* the nub stays CREAM at every deflection — "it is the thumb, not a value" (the
   ...
   thumb was the nub claiming to be a reading. ${NUB} px is the cell's 14 px
   scaled by the ring the build actually draws (r 52 → ${RING}). */
.ptouch__nub {
  position: absolute; left: 0; top: 0; width: ${NUB}px; height: ${NUB}px;
  margin: ${-NUB / 2}px 0 0 ${-NUB / 2}px; border-radius: 50%;
...
`;
  ...
  arcEl.className = 'ptouch__arc';
  const vecEl = document.createElement('div');
  vecEl.className = 'ptouch__vec';
  const dotEl = document.createElement('div');
  dotEl.className = 'ptouch__dot';
  const nubEl = document.createElement('div');
  nubEl.className = 'ptouch__nub';
  // paint order is the cell's: plate, arc, vector, anchor dot, thumb
  stickEl.append(ringEl, arcEl, vecEl, dotEl, nubEl);
  ...
  function draw() {
    ...
    // specs/0055 §4/§5 (W7) — THE HELD DIRECTION, on the ring itself.
    //
    // `--pa` is measured the way a conic-gradient measures: 0° is up, positive
    // is clockwise. Screen y grows downwards, so pushing DOWN (`ay` +1) is 180°
    // and pushing RIGHT (`ax` +1) is 90° — `atan2(ax, -ay)` is exactly that,
    // with no sign fixes anywhere else.
    //
    // The vector is the cell's construction rather than a fixed length: in the A
    // cell the 26 px line ends under a nub 26 px from the anchor, so it is the
    // deflection drawn, and at rest there is nothing to draw. Below DEAD the
    // whole direction read is off and only the plate, the anchor and the thumb
    // remain — which is the stick the instant a finger lands.
    const d = Math.hypot(stick.ax, stick.ay);
    const push = d > DEAD;
    stickEl.classList.toggle('is-push', push);
    if (push) {
      const deg = Math.atan2(stick.ax, -stick.ay) * 180 / Math.PI;
      arcEl.style.setProperty('--pa', deg.toFixed(1) + 'deg');
      vecEl.style.width = (Math.min(1, d) * RING).toFixed(1) + 'px';
      // the vector is measured from +x anticlockwise-negative like any CSS
      // rotation, so it is `--pa` turned back a quarter turn
      vecEl.style.transform = `rotate(${(deg - 90).toFixed(1)}deg)`;
    } else {
      vecEl.style.width = '0px';
    }
  }
  ...
  //
  // specs/0057 §5.1 (D12) — AND THIS IS THE GRAB ON A PHONE, with no new gesture,
  // no new chip and not one new key. Greg's ask was "should work on mobile too —
  // rotation joystick is the jump button", and it already is: the right half is
  // the jump button, its LEVEL is this line, and tricks.js reads `grab` (written here too, §4.4) while
  // airborne as the grab. The movement stick is the other half of the input and
  // is equally untouched — `aim()` below already writes `forward`/`back`/`left`/
  // `right` past PITCH/CARVE, and those are the four cardinals §5.1's table picks
  // the grab from. A stick pushed up-right while the right thumb holds is B+W
  // with a D tweak, spelled with two thumbs instead of three fingers.
  //
  // Two properties of the file as it already stands are what make that true, and
  // both are load-bearing enough to name here:
  //   1 MATES (below) — a tap is disqualified only by SAME-HALF company, so the
  //     left thumb parked on the stick for the whole run cannot eat the right-half
  //     touch. Without that fix the grab would be unreachable on a phone for
  //     exactly the reason the jump was.
  //   2 the jump EDGE fires on a TAP and this level fires on a HOLD, so tapping to
  //     leave the lip and then holding to grab are the same button used twice, in
  //     the order a rider would use it.
  //
  // What a stick CANNOT spell is §5.1's opposite-cardinal row: no thumb pushes up
  // and down at once, so TRUCK DRIVER is desktop-only. That is a property of a
  // one-anchor analog stick and not a gap in this seam — inventing a second
  // right-half finger for it would be the new gesture §5.1 rules out.
  //
  // specs/0057 §4.4 (B, Greg 2026-09-06) — B IS THE TRICK KEY NOW, AND THE GLASS
  // DOES NOT CHANGE. Everything above still holds; the only thing that moved is
  // that the grab reads `keys.grab` instead of `keys.jumpHeld`, and a phone has
  // no B to press. So this one line writes BOTH levels off the same right-half
  // hold: `jumpHeld` for the flare/preload/throttle path it has always fed, and
  // `grab` for tricks.js. The gesture, the caption and the chip count are
  // untouched — a right thumb held in the air is still the grab, and the
  // movement stick still names it.
  const syncHeld = () => {
    const held = [...touches.values()].some((v) => v.side === 'R');
    set('jumpHeld', held);
    set('grab', held);
  };
  ...
  // ---- specs/0058 · the trace chip. THE GUARD IS THE WHOLE FEATURE: the
  // recorder is stubbed out of the public build, so `window.__trace` is the
  // inert stub there and `armed()` is false forever — no element, no listener,
  // no CSS. On the bench a phone gets the same J the desktop has, because a
  // collision bug found with a thumb is the one you most want a bag of.
  const trace = (() => { try { return window.__trace || null; } catch { return null; } })();
  if (trace && trace.armed && trace.armed()) {
    // specs/0055 §8 P5 — THE TARGET GROWS, THE MARK DOES NOT PAINT ONE PIXEL
    // DIFFERENTLY. The chip was a 26 px tap target on a touch deck, which is a
    // thumb-width under the floor; but it is also drawn in C15's `--low-end`
    // lane, which diffs this build against `main` — and `main` ships this same
    // chip at 26 px (touch.js:504 there). Restyling the painted box would have
    // moved that lane by the chip's whole area, and row 10 is the one row the
    // 0055 gate reads at a pixel.
    //
    // So the button becomes the TARGET and the span becomes the MARK: the
    // button is transparent, 44 px in both axes, and pinned so its top-left
    // corner is exactly where the chip's corner already was; the span carries
    // the type, the ground, the hazard border and the padding the chip always
    // had, unchanged, at that same corner. Painted output is byte-identical to
    // `main`'s; the thumb gets 44 px. `textContent` still reads `TRACE`.
    const chip = document.createElement('button');
    chip.type = 'button';
    chip.setAttribute('aria-label', 'file a ride trace');
    chip.style.cssText = 'position:fixed;right:10px;top:10px;z-index:31;'
      + 'box-sizing:content-box;min-width:44px;min-height:44px;'
    ...
    mark.style.cssText = 'display:block;'
      + 'font:10px/1 ui-monospace,monospace;letter-spacing:.12em;color:var(--p-cream);'
      + 'background:rgba(23,22,20,.86);border:1px solid var(--p-hazard);border-radius:3px;'
      + 'padding:7px 9px;';
    chip.appendChild(mark);
    chip.addEventListener('touchstart', (e) => { e.stopPropagation(); }, { passive: true });
    chip.addEventListener('click', (e) => { e.stopPropagation(); trace.open(); });
