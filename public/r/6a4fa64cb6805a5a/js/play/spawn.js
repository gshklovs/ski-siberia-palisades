...
import { makeStandUp, standUpTuning, waypointTuning, WAYPOINT_SPAWNS } from './lift.js';
...
export const ALIASES = {
  ...
  // Greg 2026-09-05 moved the name EAGLE'S NEST onto the summit marker (id
  // `kt22`) and deleted the marker that used to hold the id `eagles-nest`. The
  // name alone does not put the slug back: slugify turns "EAGLE'S NEST" into
  // `eagle-s-nest`, so /eagles-nest — a link that worked before the rename —
  // would resolve to nothing. This is precisely what this table is for: a name
  // people type that no id spells.
  'eagles-nest': 'kt22',
};
...
export function pickSpawn(THREE, { collision, world, layout, qs, unitScale = 1, upAxis = 'y', pathname = null }) {
  ...
  // specs/0051 §3.8 — "spawn drop, spawn.js:113, boot, before any ring: PINNED
  // FIRST". The drop is the one far probe in the player that MUST get a hard
  // answer: it decides where the body starts, and a soft answer would put it on
  // the analytic field instead of on the triangles it is about to stand on.
  const drop = (x, z) => {
    if (collision.pinSync) collision.pinSync({ x, y: top, z }, 'spawn');
    return collision.groundAt(x, z, top);
  };
...
export function namedSpawn(THREE, { slug, collision, world, unitScale = 1, upAxis = 'y' }) {
  ...
  if (!wp) return null;
  const stood = standUpAt(wp.at, world, collision, unitScale, wp.id);
  if (!stood) return null;
  return {
    position: new THREE.Vector3(stood.x, stood.y, stood.z),
    yaw: stood.yaw, pitch: stood.pitch || 0,
    source: 'spawn:' + wp.id,
...
function standUpAt(at, world, collision, unitScale, wpId) {
  try {
    ...
    const standUp = makeStandUp({ collision, runs: world.runs || [], unitScale,
      terrainHeight: typeof world.terrainHeight === 'function' ? world.terrainHeight : null });
    // A WAYPOINT ROW BEATS THE LIFT ROW. `_default` is a lift unload's tuning and
    // a named drop-in is not a lift unload — see WAYPOINT_SPAWNS in lift.js.
    const row = wpId && WAYPOINT_SPAWNS[wpId] ? wpId : null;
    const T = row ? waypointTuning(row) : standUpTuning(key, keyName);
    const stood = standUp(at, T);
    if (!stood) return null;
    stood.pitch = Number.isFinite(T.pitchDeg) ? (T.pitchDeg * Math.PI) / 180 : 0;
    stood.tuned = row;
    return stood;
  } catch {
