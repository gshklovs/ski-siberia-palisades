...
// =================================================== the named-waypoint spawns
// ONE table, the same shape LIFT_SPAWNS uses, merged over the same `_default`,
// keyed by the waypoint id the world declares.
//
// `_default.maxSlopeDeg` 11 is A LIFT UNLOAD'S NUMBER. It is what stops the ring
// search standing you on a cliff you did not ask for when you step off a chair,
// and it is exactly wrong for a waypoint whose entire name is a drop-in: The
// Nose's face is 33 deg, which the ceiling docks 6 points a degree for (-132),
// so the search walked 50 m up-ridge to a 10.5 deg shelf and Greg's own dot was
// never even a candidate. A row here says "this place IS steep, that is the
// point of it", and it says so per waypoint rather than by loosening the number
// every chair unload in the world depends on.
//
// `yawDeg` pins the heading (`headingFor` already honours it); `pitchDeg` is
// read by spawn.js and by markers.js's fast travel, and is the one field the
// lift table has no use for.
export const WAYPOINT_SPAWNS = {
  'the-nose-drop-in': {
    rings: [0, 5, 10], probes: 12,
    targetSlopeDeg: 33, maxSlopeDeg: 40,   // the face, not a bench
    minDropM: 25, homeBiasPerM: 1.6,       // and stand on the DOT, not near it
    yawDeg: -18.24,   // bearing 18.24 deg to the centre of FIN_CHUTES' five tops
    // ...and 21.5 deg DOWN. The reef's own centre sits 28.9 deg under the
    // horizon from the dot, so this puts it 7.4 deg under the view axis — just
    // below the middle of the frame — and the ground 25 m ahead, which is the
    // drop-in you are standing on the lip of, at 70 % down: the lower third, by
    // projection rather than by eye (work/nose_spawn_probe.mjs prints both).
    pitchDeg: -21.5,
  },
};
export function waypointTuning(id) {
  return { ...LIFT_SPAWNS._default, ...(WAYPOINT_SPAWNS[id] || {}) };
}
...
export function makeStandUp({ collision, runs, unitScale = 1, terrainHeight = null }) {
  // the unconditional probe — used by the spawn search, where "no ground here"
  // is a real answer and must not be papered over.
  //
  // specs/0051 §3.6 — THE BOOT-TIME WHOLE-WORLD DEPENDENCY, and what actually
  // removed it. `LIFT_SPAWNS`' unload search walks the fall line at all seven
  // lift tops, at BOOT, kilometres from spawn; §3.6 says to move it to
  // `world.terrainHeight` because "the walk wants an analytic height, not a
  // triangle raycast", and that "removes the boot-time dependency on a
  // whole-world collision grid".
  //
  // The dependency is removed — by the ROUTER, which builds the handful of
  // tiles these probes touch on demand and never a whole-world grid (measured:
  // `__chunks.softHits` is 0 through a boot that runs this search at all seven
  // tops). The analytic swap itself is NOT taken, because it was measured and it
  // moves the answer: with `terrainHeight` here, red-dog's derived unload goes
  // from [344.7, 384.4] yaw 48° to yaw 18.0°, and `verify.mjs`'s boot-spawn
  // cross-check — the probe §3.6 says this "fixes for free" — reads Δ 90.5°
  // against its 90° cap and FAILS. The reason is real: the shipped terrain
  // interpolates groundZ on a 1.13-3.40 m ladder, so a cut cat track is a
  // smoothed bench on the mesh the skier stands on and a sharp trench in the
  // analytic field, and the fall line on those two surfaces genuinely differs.
  //
  // So the triangles stay the source of truth for the ground you stand on, and
  // `terrainHeight` is the FALLBACK underneath them — which is exactly the soft
  // answer §3.6 defines, taken here explicitly rather than through the router's
  // counter. See PROGRESS-0051-2c.md, deviation 2.
  const gAt = (x, z) => {
    const g = collision.groundAt(x, z, collision.bounds.maxY + 5 * unitScale);
    if (g !== null) return g;
    if (typeof terrainHeight !== 'function') return null;
...
export function createLifts({ THREE, lifts, collision, ctrl, hud, scene, runs, unitScale = 1, onRide, terrainHeight = null }) {
  ...
  function groundY(x, z, hintY) {
    // specs/0051 §3.6/§3.8 — the boarding decal settles on real triangles at a
    // lift base kilometres from spawn. Pin the destination first so the probe
    // gets a HARD answer off a resident slab instead of the soft one.
    if (collision.pinSync) collision.pinSync({ x, y: hintY, z }, 'lift-decal');
  ...
  const standUp = makeStandUp({ collision, runs, unitScale, terrainHeight });
  ...
  function ride(L) {
    ...
    // specs/0056 §4 — a lift ride is the second of the two things that clear the
    // snow you are carrying (R is the other). It goes AFTER the teleport, which
    // already put the gear back on the body, so the rider unloads at the top
    // clean. There is no separate unload event to hang it on: the ride IS the
    // teleport (see this file's header), so boarding and arriving are one frame.
    if (ctrl.snowReset) ctrl.snowReset('lift');
    rides++;
