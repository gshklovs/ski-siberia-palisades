...
// specs/0041 §4.3 (A17): the signed skate/pole push (−1 / 0 / +1) AS THE :884 GATE ADMITS IT this step — 0
// when airborne, braking, or already above S.skateMax. `push` itself is a local of skiStep (critic #25), so
// this is the one capture that lets skiState() export it; `skating === (skiState().push !== 0)` is the gate's
// own expression, which is why one field carries both of §4.3's rows.
let _push = 0;
...
// specs/0060 — HOW WELL TIMED that pop was, 0..1, latched on the frame it is
// paid. A pure read-out: nothing in this file consumes it and the physics is
// untouched by construction. main.js's hitstop is its only reader.
let _popQ = 0;
...
// ---- specs/0057 §3.2 (D6): "rail slightly charges compression so you get good
// launch off". `_railComp` is rail.js's bank, in the SAME units the compression
// is measured in (m/s of surface rate above the running floor), and it is a
// NAMED ADDITIVE TERM: it is summed into the compression at the two places the
// compression is read — `_lipComp` below and `popPay()` — and nowhere else.
//
// Nothing about the existing model moves. `lipCompK` 0.25, `lipCompMax` 3.0,
// `popCompK` 0.18 and `popCompMax` 3.0 keep their values and their caps, so a
// pop off a jib is spent through exactly the arithmetic a pop out of a snow
// compression is, and D6's "the existing charge cap / pop model is unchanged"
// is a statement about this file rather than a promise about a new one. With no
// jib under the rider `_railComp` is 0 and both read sites are bit-for-bit the
// expression they were.
let _railComp = 0;
export function setSkiRailComp(v) { _railComp = Number(v) > 0 ? Number(v) : 0; return _railComp; }
// The compression the ski is carrying: what the ground built up, plus what the
// jib banked. One expression, two callers, so the two can never disagree.
const compNow = () => Math.max(0, _vyS - _vyFloor) + _railComp;
...
export function skiState() {
  return {
    ...
    releasing: _pumpRelT > 0, relT: _pumpRelT, payout: _pumpPay,
    // branch rider/pump-oneshot — the TURN CLOCK, live. `_pumpTurnT` is how long this turn has
    // been going and `_pumpTPeak` the time on that clock at which the edge load peaked; the
    // transition's `timing` term is already `1 − |turnT − tPeak| / pumpWindow` (:859). rider.js
    // reads the two to find the APEX — the frame `pumpPhase` crosses from completion to
    // initiation with the peak still inside `S.pumpWindow` — and fires its one-shot there. READ
    // ONLY: nothing in this module is computed from these two lines and the bank, the payout and
    // every launch number are byte-for-byte what they were.
    turnT: _pumpTurnT, tPeak: _pumpTPeak,
    ...
    // specs/0060 — the LAST paid pop's timing quality, 0..1. Read on the rising
    // edge of `popPaid` and meaningless at any other moment.
    popQuality: _popQ,
    ...
    // ---- specs/0041 §4.3 (A17): the skate push the :884 gate admitted this step, −1/0/+1 (0 = not skating)
    push: _push,
  };
}
...
export function skiStep(ctx) {
  ...
  //
  // specs/0065 — AND THE ARROWS ARE NOT PART OF IT. They used to be exact aliases
  // of W/S on the snow (`ctx.grounded && keys.flipFwd`); on the ground they now
  // aim the view instead (camRig's `look`, main.js), and in the air they are still
  // the trick axis (spec §3.1). So the push is W and S alone — which is what it
  // was before 0002 gave the arrows a second job — and every skate/brake/switch
  // behaviour below, all of it built on sign(vf), is untouched.
  const pushF = !!keys.forward;
  const pushB = !!keys.back;
  const push = (pushF ? 1 : 0) + (pushB ? -1 : 0);
  _push = 0;                                    // §4.3: re-armed by the skate gate below, else stays 0
  ...
  if (ctx.grounded) {
    ...
    if (push !== 0 && !brake && Math.abs(vf) < S.skateMax) {
      _push = push;                             // §4.3 `skating`: the gate is live this step
    ...
    _lipComp = compNow() * S.lipCompK;   // specs/0057 §3.2 — + the jib's bank
...
function popPay(S) {
  const comp = compNow();               // specs/0057 §3.2 — + the jib's bank
  if (_lipVy > 0) {
...
export function skiLaunch(vel, S, popVy) {
  ...
  _popPaid = true;
  // specs/0060 — and how well timed it was, on whichever of the three windows
  // actually admitted it: a pop firing ON the snow at the lip is 1, a coyote
  // one decays over `popCoyote`, a pre-lip one over `popWindow`.
  _popQ = popVy > 0
    ? (_airT > 0 ? 1 - clamp01(_airT / (S.popCoyote || 1)) : 1)
    : 1 - clamp01(late / (S.popWindow || 1));
