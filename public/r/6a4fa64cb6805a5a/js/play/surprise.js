...
// specs/0055 §1.7 — the notice board reads the shared kind palette instead of
// typing its own hexes. This is exactly what exporting KINDS was for: the rope
// is `venue`'s ink and the two clips are `venue`'s panel — the amber a resort
// paints an official notice in — and no panel types a hex after §1.7.
import { KINDS } from './markers.js';

const QUIP_CD = 2.6;        // s — one crash, one line
// specs/0055 §4.7 (fidelity 2026-09-06) — how long a tree quip waits to find
// out whether it was a brush or a crash. It is §4.7's own 0.40 s, the gap the
// wipeout panel already uses between its word and its explanation.
const TREE_HOLD = 0.40;     // s
// specs/0055 §6, §3.11 (Greg 2026-09-06) — the picked cell's own dwell: 4.2 s
// (up from 2.5 s) PLUS 45 ms per word, because a hung board carries a whole
// sentence of prose rather than a chip you read in a glance and a long line has
// to be readable at the same pace as a short one. It leaves on a 160 ms FALL —
// the one exit verb in the system — instead of the old 300 ms slide.
const TOAST_MS = 4200;      // toast lifetime (HOLD), before the per-word add
const TOAST_PER_WORD = 45;  // ms — §3.11's "+45 ms per word"
const TOAST_OUT = 160;      // FALL, §1.11
...
const S = {
  ...
  // specs/0055 §4.7 (fidelity 2026-09-06) — the tree quip's 0.40 s hold. A line
  // waits here until the frame proves it was a BRUSH and not a CRASH; a rising
  // `wipeT` inside the hold cancels it, because that event already has a voice.
  pend: null,
...
/* specs/0055 §3.11 (Greg 2026-09-06) — PATROL: A, THE ROPE LINE.
//
// AS PICKED: "Id rather go with the hanging sign instead of the bottom safety
// for the ski patrol banners." A replaces D — the boundary tape is gone, whole,
// hatch and all. What ships is the object real patrol hangs on a hill: a board
// clipped to a rope.
//
// THE ROPE IS THE MOUNTING RULE, moved to the edge the board actually hangs
// from — the same geometry as every other panel's rule, rotated 180 deg by the
// physics of the thing. That is the whole idea, and it is why the board's own
// bottom edge carries nothing: a hung sign is held at the top.
//
// NO NEW SLAB (§1.6, D19). The board is the existing panel surface — cream
// ground, ink type (`--p-cream` / `--p-ink`, KINDS['ski-run'].panel by another
// name). The two clips are the only colour on it and they are
// `KINDS.venue.panel`, the amber a resort paints an official notice in — the
// same constant D's tape read, so the amber survives the swap by name and this
// module still types no hex (§1.7).
//
// §1.2's caps/roman split is BACK. D broke it on purpose ("on real tape
// everything is caps"); a board is a board, so the header is mono caps and the
// line is roman prose, exactly as the picked cell draws it. */
const K_NOTE = KINDS.venue;
const CSS = `
.psur { position: fixed; left: 0; right: 0; bottom: 26px; z-index: 21;
  display: flex; flex-direction: column-reverse; align-items: center; gap: 10px;
  pointer-events: none; }
/* the cell draws the notice 434 px wide; the cap is the only thing the cell
   cannot know, because a lookbook stage has no viewport to overflow */
.psur__toast { width: 434px; max-width: calc(100% - 32px); }
.psur__toast.is-out { animation: psurOut ${TOAST_OUT}ms linear both; }

/* ---- the rope. 3 px of ink, and the two clips are its ::before/::after, so
   the DOM carries no element that is purely decoration. WIPE (§1.11): the rope
   DRAWS left to right over 110 ms and the board follows it. */
.psur__rope {
  position: relative; height: var(--p-spine, 3px); background: ${K_NOTE.ink};
  box-shadow: 0 1px 3px rgba(0, 0, 0, .45);
  transform: scaleX(0); transform-origin: left center;
  animation: psurDraw var(--p-wipe-rule, 110ms) cubic-bezier(.16, 1, .3, 1) both;
}
.psur__rope::before, .psur__rope::after {
  content: ''; position: absolute; top: 2px; width: 9px; height: 13px;
  background: ${K_NOTE.panel}; box-shadow: 0 1px 3px rgba(0, 0, 0, .4);
}
.psur__rope::before { left: 34px; }
.psur__rope::after { right: 34px; }
/* a rare line has nowhere to go but the clips — they are the only colour on the
   panel — so it lifts them to the kind's glow, one step up the same amber
   (specs/0012 §C — no left stripe on any banner) */
.psur__toast.is-rare .psur__rope::before,
.psur__toast.is-rare .psur__rope::after { background: ${K_NOTE.glow}; }

/* ---- the board, hung 13 px under the rope. It UNMASKS DOWNWARD from behind
   the rope over 260 ms — it is hung, not flown in — so the clip runs after the
   rope has finished drawing. */
.psur__board {
  margin-top: 13px; padding: 8px 15px 10px;
  /* specs/0055 §10.3 — THE ONE LINE IN THIS FILE WITH NO FALLBACK, and the
     reason is the sweep, not taste: the cream is one of the three hexes §10.3
     reserves to hud.js and markers.js KINDS, and a var() fallback types a hex
     just as surely as a declaration does. This module reads the two surfaces
     by name and spells neither. Nothing is lost — W1's :root block is
     published by hud.js before any toast can hang — and every other token
     below keeps its fallback, because none of those values is reserved. */
  background: var(--p-cream); color: var(--p-ink);
  border-radius: 0 0 var(--p-r, 2px) var(--p-r, 2px);
  box-shadow: 0 9px 26px rgba(0, 0, 0, .44);
  animation: psurHang var(--p-wipe-body, 260ms) cubic-bezier(.16, 1, .3, 1)
    var(--p-wipe-rule, 110ms) both;
}
/* the header is a LEDGER OF TWO: what said it, and where and when */
.psur__hd {
  display: flex; justify-content: space-between; align-items: baseline; gap: 18px;
  border-bottom: var(--p-hairline, 1px) solid var(--p-seam, #c8c2b3);
  padding-bottom: 5px; margin-bottom: 6px;
}
.psur__what {
  font-family: var(--p-mono, ui-monospace, Consolas, monospace);
  font-size: 10.5px; font-weight: 400; font-style: normal; line-height: 1.55;
  letter-spacing: .13em; text-transform: uppercase; white-space: nowrap;
}
.psur__what--r { color: var(--p-sub, #726c60); }
.psur__ln {
  display: block;
  font-family: var(--p-fam, "Avenir Next", Avenir, "Nunito Sans", "Segoe UI", system-ui, sans-serif);
  font-size: var(--p-prose, 13px); font-weight: 400; font-style: normal;
  line-height: 1.44; letter-spacing: 0; text-transform: none;
}
@keyframes psurDraw { to { transform: scaleX(1); } }
/* downward, from behind the rope: the bottom inset retreats to zero */
@keyframes psurHang {
  from { clip-path: inset(0 0 100% 0); }
  to   { clip-path: inset(0 0 0 0); }
}
/* FALL: 160 ms of straight opacity on the whole notice. The board and the rope
   go together and the rope does NOT un-draw — you do not un-hang a sign. */
@keyframes psurOut { 0% { opacity: 1 } 100% { opacity: 0 } }
@media (prefers-reduced-motion: reduce) {
  .psur__rope, .psur__board, .psur__toast.is-out { animation: none; }
  .psur__rope { transform: none; }
}
`;
...
// specs/0055 §3.11 (Greg 2026-09-06) — the header's right-hand token: WHERE and
// WHEN, the second half of the cell's ledger of two. Both come off state this
// module already keeps — the matched registry key and the run clock it already
// formats for two of its own lines — so the picked cell costs the registry no
// new field. The `.psur__what` rule uppercases, so the key needs no casing pass.
function noticeWhere() {
  const k = S.key || S.poi || '';
  const place = k.replace(/-/g, ' ').trim();
  const clock = fmtTime(S.runT);
  return place ? place + ' · ' + clock : clock;
}

// With the tag standing in the header, a line that opens by saying its own tag
// says it twice. Exact prefix only — "PATROL RADAR: 61 km/h…" loses the label,
// "THE ROCK RATES THAT SPLASH: 8/10" keeps every word, and no row in REG is
// edited to make the panel work.
function stripTag(text, tag) {
  if (!tag) return text;
  const p = tag + ': ';
  return text.slice(0, p.length).toUpperCase() === p.toUpperCase()
    ? text.slice(p.length) : text;
}
...
function toast(text, tag, rare) {
  try {
    ...
    const label = tag || 'PATROL';
    const line = stripTag(text, label);
    const el = document.createElement('div');
    el.className = 'psur__toast' + (rare ? ' is-rare' : '');
    // The rope carries its own two clips as ::before / ::after, so the DOM holds
    // no element that is purely decoration — the same rule D's hatch followed.
    // A row with no tag still gets the left-hand token: an unheaded board reads
    // as a graphic rather than as a notice, and PATROL is the fallback because
    // this is the panel patrol speaks through.
    const rope = document.createElement('div');
    rope.className = 'psur__rope';
    el.appendChild(rope);
    const board = document.createElement('div');
    board.className = 'psur__board';
    const hd = document.createElement('div');
    hd.className = 'psur__hd';
    const l = document.createElement('span');
    l.className = 'psur__what';
    l.textContent = label;
    hd.appendChild(l);
    const r = document.createElement('span');
    r.className = 'psur__what psur__what--r';
    r.textContent = noticeWhere();
    hd.appendChild(r);
    board.appendChild(hd);
    const f = document.createElement('span');
    f.className = 'psur__ln';
    f.textContent = line;
    board.appendChild(f);
    el.appendChild(board);
    S.root.appendChild(el);
    // HOLD is per notice: 4.2 s plus 45 ms a word, so the long lines get the
    // reading time the short ones do not need.
    const words = line.split(/\s+/).filter(Boolean).length;
    S.live.push({ el, t: 0, hold: TOAST_MS + TOAST_PER_WORD * words });
    S.lastQuip = text;
  } catch { S.errors++; }
}
...
function tickToasts(dt) {
  for (let i = S.live.length - 1; i >= 0; i--) {
    const r = S.live[i];
    r.t += dt * 1000;
    const hold = r.hold || TOAST_MS;
    if (r.t > hold - TOAST_OUT && !r.out) { r.out = true; r.el.classList.add('is-out'); }
    if (r.t > hold) killToast(r);
  }
}
...
// specs/0055 §3.11 — WIRE THE RED DOG ROWS TO THE RUN THEY DESCRIBE.
//
// `poiKey()` matches `ctx.poi` exactly, or a REG key that is a PREFIX of
// `ctx.run`. This world's run id is `palisades-front-A-merge-01`, and
// `red-dog-palisades` is a prefix of nothing — so the five rows above have
// never once fired on the world they were authored for, which is why §3.11 could
// say the registry "has no key matching this run". The rows are right; the key
// is the wrong shape. One alias fixes it, and it aliases rather than renames so
// `?poi=red-dog-palisades` keeps working.
//
// THIS IS THE ONE-LINE DATA CHANGE §3.11 HELD BACK. It landed with D and it
// stays: specs/0055 §3.11 (Greg 2026-09-06) swapped the panel to A, the rope
// line, and a variant swap is not a reason to un-wire the rows it draws.
REG['palisades-front'] = REG['red-dog-palisades'];
...
function checkTrees(dt) {
  ...
  // specs/0055 §4.7 (fidelity 2026-09-06) — a rising `wipeT` is the frame the
  // wipeout title cut takes, so a tree hit detected on THAT edge is the same
  // event the stamp is already captioning. It still counts (the streak and the
  // hit tally are the tree pool's own facts); it does not get a second line.
  const wipeEdge = wipe > 0 && S.prevWipe <= 0;
  const crashed = wipeEdge || (S.prevSpeed > 5 && decel > 45);

  // ---- specs/0055 §4.7 (fidelity 2026-09-06) — THE 0.40 s HOLD, and it is
  // §4.7's own beat. A tree quip fires on the frame the body enters the stem's
  // corridor, which for an actual crash is one or two frames BEFORE `wipeT`
  // rises — so a same-frame test alone still let the quip caption the wipeout,
  // which is what Greg saw. The line is chosen on that frame and HELD: if a
  // wipe starts inside the hold the line is dropped (the title cut has the
  // frame, and one event gets one voice), and if none does it prints, which is
  // every ordinary run through a stand of trees.
  if (S.pend) {
    if (wipe > 0) S.pend = null;
    else {
      S.pend.t -= dt;
      if (S.pend.t <= 0) { toast(S.pend.line, S.pend.tag, S.pend.rare); S.pend = null; }
    }
  }
  ...
  if (S.quipCd <= 0 && (speed > 5.5 || crashed)) {
    ...
    if (i >= 0) {
      ...
      if (!wipeEdge) S.pend = { line, tag: S.streak > 1 ? 'TREE × ' + S.streak : 'TREE', rare, t: TREE_HOLD };
    }
  }
...
function checkSolids() {
  ...
  // ---- specs/0055 §4.7 (fidelity 2026-09-06) — ONE VOICE PER EVENT, and on a
  // wipeout that voice is the title cut. Greg, 2026-09-06: "we still have old
  // claudish text showing up describing the trick even though banners are
  // coming." This detector fires on ONE rising edge of `wipeT` — the exact
  // frame `hud.trick()` stamps WIPEOUT with its own subtitle from the copy
  // table — so its quip is a second caption on the same beat, and it goes.
  //
  // NOTHING ELSE HERE CHANGES: the streaks, the hit counts, `S.lastSolid` and
  // the 60 s window are all still kept, so `_test.pump()` still reads the real
  // detector and the free-skiing tree quips (which do not ride a wipe edge) are
  // untouched. The line is chosen and recorded; it is simply not printed over
  // the panel that has the frame.
}
