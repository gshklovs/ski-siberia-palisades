...
class StemSet {
  constructor() {
    ...
    this.meshes = [];
    this.n = 0;                              // high-water slot index (NOT the live count)
    this.live = 0;                           // live stems — `n` minus the free list
    this.free = [];                          // slots freed by `removeAt`, reused by `addLive`
  ...
  index() {
    ...
    for (let i = 0; i < this.n; i++) {
      if (this.sr[i] <= 0) continue;                 // a freed slot indexes nothing
      this.gridAdd(i);
    }
  }

  // ======================================================= streamed stems
  // specs/0051 §5.3 moved the firs out of world.mjs's whole-world instancing
  // and into the residency scheduler's POOL_INST, and `scan()` below is a
  // ONE-SHOT traverse cached per scene root — so after wave 3a the only stems
  // left in red-dog were the 70 `snags` world.mjs still instances itself, and
  // every fir in the world was pass-through. Measured on the two benches,
  // 2026-09-06: unchunked main 24,470 stems, chunked chunk/w0 **70**.
  //
  // The set therefore has to be MUTABLE: a tile's trunks are added when the
  // tile lands and dropped when it is evicted, which is also what makes a
  // teleport correct (the destination's tiles land through the same path).
  //
  // Three properties this has to keep, and each one is bought here rather
  // than hoped for:
  //   * `stemHit`/`canopyIn` walk `grid` buckets, so a slot has to leave its
  //     bucket the instant it is freed — an index left behind reads a stale
  //     trunk at a place there is no longer a tree.
  //   * the arrays are FIXED at MAX_STEMS and are never reallocated, so a
  //     freed slot goes on a free list and is reused. `n` is the high-water
  //     index, not the live count; `live` is the live count.
  //   * `maxR`/`maxCR` are the query REACH. They are high-water marks and are
  //     never lowered: shrinking them on a free could make `stemHit` stop
  //     looking at a cell that still holds a wider trunk. They cost one extra
  //     cell of scan, which is O(cell) either way.
  gridKey(i) { return Math.floor(this.sx[i] / CELL) + ',' + Math.floor(this.sz[i] / CELL); }

  gridAdd(i) {
    const k = this.gridKey(i);
  ...
  gridDrop(i) {
    ...
    a[at] = a[a.length - 1]; a.pop();
    if (!a.length) this.grid.delete(k);
    return true;
  }

  /** `add()` plus the hash insert, for a set that is already indexed. Returns
   *  the stem index, or -1 when the set is full. */
  addLive(x, y, z, h, r, top, cy = 0, cr = 0, sm = -1, si = -1) {
    let i;
    if (this.free.length) {
      i = this.free.pop();
      this.sm[i] = sm; this.si[i] = si;
      ...
      this.st[i] = (top > y + 0.05) ? top : (y + Math.max(0.5, h));
      this.cy[i] = cy;
      const c = cr * CANOPY_R_SCALE;
      this.cr[i] = (c > 0.05 && this.st[i] > cy + 0.5) ? c : 0;
      if (r > this.maxR) this.maxR = r;
      if (this.cr[i] > this.maxCR) this.maxCR = this.cr[i];
      this.live++;                            // `add()` counts its own; this branch does not
    } else {
      i = this.add(x, y, z, h, r, top, cy, cr, sm, si);
      if (i < 0) return -1;
    }
    this.gridAdd(i);
    return i;
  ...
  /** Free one stem: out of the hash, onto the free list, and made inert so a
   *  bucket that somehow still names it can never report a contact. */
  removeAt(i) {
    if (i < 0 || i >= this.n || this.sr[i] <= 0) return false;
    this.gridDrop(i);
    this.sr[i] = 0; this.cr[i] = 0; this.sh[i] = 0;
    this.st[i] = this.sy[i] - 1e6;      // vertical gate can never be satisfied
    this.sm[i] = -1; this.si[i] = -1;
    this.free.push(i);
    this.live--;
    return true;
  }
  ...
  // ---- specs/0051 D-20: THE WHOLE STAND, not the deepest trunk.
  //
  // `stemHit` above answers with ONE contact — the deepest — and that is the
  // right answer for a rider meeting a tree. It is the wrong answer for a rider
  // standing in a POCKET: at `COLLIDE_KEEP` 1.0 the reference stand holds 400
  // stems per hectare and fourteen trunks inside 6 m of the body, and
  // controller.js's three passes of "push out of the deepest one" push out of A
  // into B and out of B into A until they run out of passes and leave the body
  // inside the wood. Measured, D-19: 0.00 m/s for ten seconds.
  //
  // So the guard gets to see the whole set and solve against it. This is the
  // SAME walk as `stemHit` — same cells, same vertical gate, same taper, same
  // reach — collecting instead of keeping the maximum, so it costs what
  // `stemHit` costs and not more. The array is the caller's: the controller
  // owns one and reuses it every step, so a hot path that runs on every frame
  // of every body allocates nothing.
  //
  // Deepest-first, because a Gauss-Seidel relaxation that starts on the worst
  // overlap converges in fewer passes than one that starts on a graze.
  //
  // Returns the number of contacts written (<= `max`). Each is
  // `{ i, nx, nz, pen, r, dist }`, exactly `stemHit`'s shape.
  stemContacts(x, y, z, bodyR, unit = 1, out = [], max = 16) {
    if (!this.n) return 0;
    const TOL = STEM_BASE_TOL * unit;
    const reach = bodyR + this.maxR * unit;
    const i0 = Math.floor((x - reach) / CELL), i1 = Math.floor((x + reach) / CELL);
    const j0 = Math.floor((z - reach) / CELL), j1 = Math.floor((z + reach) / CELL);
    let m = 0;
    for (let cx = i0; cx <= i1; cx++) {
      for (let cz = j0; cz <= j1; cz++) {
        ...
        for (let k = 0; k < a.length; k++) {
          ...
          const d = Math.sqrt(d2);
          const pen = r - d;
          // the set is small; an insertion over <= 16 slots beats allocating an
          // array to sort, and it is what lets `max` drop the SHALLOWEST
          // contact rather than whichever one happened to arrive last.
          let at = 0;
          while (at < m && out[at].pen >= pen) at++;
          if (at >= max) continue;                    // shallower than everything kept
          if (m < max) { if (!out[m]) out[m] = {}; m++; }
          const e = out[m - 1];                       // the slot falling off the end
          for (let s = m - 1; s > at; s--) out[s] = out[s - 1];
          out[at] = e;
          e.i = i; e.pen = pen; e.r = r; e.dist = d;
          if (d > 1e-6) { e.nx = dx / d; e.nz = dz / d; } else { e.nx = 1; e.nz = 0; }
        }
      }
    }
    return m;
  ...
  /** ONE stem's contact at (x, y, z), written into the caller's `out`. Returns
   *  the penetration (0 = none). The solver's re-measure: a Gauss-Seidel pass
   *  has to ask again after every correction it applies, and asking about a
   *  stem it already holds the index of is arithmetic, not a query. */
  stemProbe(i, x, y, z, bodyR, unit = 1, out) {
    if (i < 0 || i >= this.n || this.sr[i] <= 0) return 0;
    const TOL = STEM_BASE_TOL * unit;
    if (y < this.sy[i] - TOL || y > this.st[i]) return 0;
    const r = this.trunkRadiusAt(i, y, unit) + bodyR;
    const dx = x - this.sx[i], dz = z - this.sz[i];
    const d2 = dx * dx + dz * dz;
    if (d2 >= r * r) return 0;
    const d = Math.sqrt(d2);
    if (out) {
      if (d > 1e-6) { out.nx = dx / d; out.nz = dz / d; } else { out.nx = 1; out.nz = 0; }
    }
    return r - d;
  }

  /** Is (x, y, z) clear of EVERY trunk? The eject's own test — it walks a line
   *  and needs one boolean per sample, not a contact set. */
  stemClear(x, y, z, bodyR, unit = 1) {
    return this.stemHit(x, y, z, bodyR, unit) === null;
  }
  ...
  stats() {
    return { stems: this.live, slots: this.n, free: this.free.length,
             sources: this.sources.slice(), errors: this.errors, ms: this.ms };
  }
}
...
// ===========================================================================
// THE STREAMED-TREE PATH — specs/0051 §5.3 / D-19, and the fix for "i can
// phase through trees in deployed version" (Greg, 2026-09-06).
//
// `scan()` below reads an InstancedMesh that is ALREADY FULL: it measures the
// class off instance 0 and walks `o.count`. A pooled tile forest is neither —
// at the moment the router harvests, `chunks:forest`'s eleven meshes hold
// `count = 0` and instance 0's matrix is all zeros, so `scan()`'s own
// `treeish`/`e.up` gates reject them and would go on rejecting them however
// many times it were re-run.
//
// So the tile path measures the PROTOTYPE (the geometry, which is what a
// class actually is) once per mesh, and then does the per-instance arithmetic
// `scan()` does — the same box-row apex, the same `profFrac` foliage line, the
// same `STEM_R x scale` trunk — off the composed matrix of one instance. One
// implementation of "what is a fir made of", two ways of being handed a fir.
//
// `sm`/`si` are deliberately LEFT AT -1 on a streamed stem. canopy.js's
// `rustle()` writes a swayed matrix straight into `instanceMatrix` and
// restores it afterwards; POOL_INST rewrites those same bytes on every tile
// swap, so a sway that outlives its tile would restore a stale matrix over a
// live instance. The rest of the canopy fx — the snow burst, the needles,
// the drag cone — reads `sx/sy/cy/st/cr` only and is unaffected.
const TILE_PROTO = new WeakMap();

/** Everything about ONE pooled prototype that does not change per instance.
 *  Memoised on the mesh. Returns null for a mesh that is not a tree. */
export function stemProtoOf(THREE, im) {
  if (!im || !im.geometry) return null;
  let p = TILE_PROTO.get(im);
  ...
  try {
    ...
    if (bb && named(im)) {
      // parents first: the pool's forest group hangs under `play:zup`, and on
      // the first tile of a boot nothing has rendered yet, so `matrixWorld`
      // would otherwise be identity and the up axis would read as Y.
      im.updateWorldMatrix(true, false);
      const up = upAxisOf(im.matrixWorld);
      const prof = treeProfile(g, up);
      const span = prof.u1 - prof.u0;
      p = {
        bcx: (bb.min.x + bb.max.x) / 2, bcy: (bb.min.y + bb.max.y) / 2, bcz: (bb.min.z + bb.max.z) / 2,
        bhx: (bb.max.x - bb.min.x) / 2, bhy: (bb.max.y - bb.min.y) / 2, bhz: (bb.max.z - bb.min.z) / 2,
        profFrac: span > 1e-6 ? (prof.uCan - prof.u0) / span : CANOPY_FALLBACK_Y0,
        rMax: prof.rMax,
        wsc: xzScaleOf(im.matrixWorld),
      };
    }
  } catch { p = null; }
  TILE_PROTO.set(im, p);
...
/** One streamed instance, from its composed world matrix `CM`
 *  (= `im.matrixWorld * instanceMatrix`). Returns the stem index or -1.
 *
 *  specs/0051 D-20 — `canopy` splits the two keeps. The TRUNK is collision and
 *  goes to full density; the FOLIAGE is specs/0012 §E2's soft tax, whose
 *  `canopyEntry`/`canopyDrag` Greg fitted twice on the bench (specs/0031 §1,
 *  0032 §1) against placeForest's 144 stems/ha. A tree outside the canopy
 *  subset registers with `cr = 0`, which `canopyIn()` already skips — the drag
 *  cone stays exactly the forest it was tuned on while every drawn tree becomes
 *  solid. No new branch anywhere; one argument. */
export function addStemFrom(set, proto, CM, canopy = true) {
  if (!set || !proto) return -1;
  const m = CM.elements;
  const x = m[12], y = m[13], z = m[14];
  // world Y of the transformed geometry box — the same two lines `scan()`
  // uses, and for the same reason: an affine map sends the box centre to `cy`
  // and its extent to the L1 norm of the matrix's Y row against the half-sizes.
  const cy = m[1] * proto.bcx + m[5] * proto.bcy + m[9] * proto.bcz + m[13];
  const hy = Math.abs(m[1]) * proto.bhx + Math.abs(m[5]) * proto.bhy + Math.abs(m[9]) * proto.bhz;
  const apex = cy + hy, foot = cy - hy;
  const scl = Math.hypot(m[0], m[1], m[2]);     // the composed xz scale, world included
  return set.addLive(x, y, z, Math.max(0.5, apex - foot), STEM_R * scl, apex,
                     foot + (apex - foot) * proto.profFrac,
                     canopy ? proto.rMax * scl : 0, -1, -1);
}
