...
// specs/0055 §1.1, §10.3 — the signal colour by name, not by hex. The touch
// diagram is drawn in inline SVG, whose presentation attributes do not take
// var(), so it reads the same object the :root block is published from.
...
function keyList() {
  ...
  // specs/0061 (M opens the map, 2026-09-06) — SIX rows now. M is a shipped key
  // (it opens the locker on the trail tab), and the rule this list is built on
  // is that a shipped key is on both lists or on neither. It is last because it
  // is the newest, and hud.js appends its own M row after the five for the same
  // reason: the two lists are still the same list, in the same order.
  const ROWS = [
    ['ESC', 'settings'], ['W A S D', 'move'], ['← →', 'tricks in the air'],
    // specs/0065 — ↑ ↓ aim the view on the ground (they are flips in the air, which
    // is the row above's job). Position and wording are the hud.js LOOK_ROWS row
    // exactly: verify.mjs asserts this card and the ESC panel against ONE list.
    // specs/0057 §4.4 (B, Greg 2026-09-06) — B · grab, after ↑ ↓ and before M,
    // in hud.js's GRAB_ROWS position exactly. B is a shipped key now (it is the
    // trick key, `keys.grab`), and the rule this list is built on is that a
    // shipped key is on both lists or on neither.
    ['C', 'camera'], ['R', 'reset'], ['↑ ↓', 'look'], ['B', 'grab'], ['M', 'trail map'],
  ];
...
// in the HUD's own language — the instrument plate, the signal accent, the
...
`));
  ...
  const stick = svg(`
    ...
    <circle cx="34.5" cy="22.5" r="7.5" fill="${HOT}"/>
  ...
  const look = svg(`
    <circle cx="19" cy="20" r="5.5" fill="${HOT}"/>
