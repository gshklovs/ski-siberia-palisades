...
// specs/0055 §1.1, §10.3 — the tokens, by name. This file painted six
// literals of cream / ink / signal orange into canvas textures and typed nine
// more into its stylesheet; the stylesheet reads --p-* now and the canvas reads
// the same objects those names are published from, so guide.js types no hex.
import { hudSurf } from './hud.js';
...
function signTexture(T, { name, sub, diff, arrow }) {
  ...
  g.fillStyle = hudSurf.ink; g.fillRect(0, 0, W, 56);
  g.fillStyle = hudSurf.cream;
  g.font = '600 26px ui-monospace, Menlo, monospace';
  g.textBaseline = 'middle';
  g.fillText(String(sub || 'TRAIL').toUpperCase(), 22, 29);
  // difficulty pip
  const pip = { blue: '#1450c8', black: hudSurf.ink, double: hudSurf.ink, green: '#2f8f3f' }[diff] || '#1450c8';
  g.fillStyle = pip;
...
function bannerTexture(T, text, sub) {
  ...
  c.width = W; c.height = H;
  const g = c.getContext('2d');
  g.fillStyle = hudSurf.ink; g.fillRect(0, 0, W, H);
  g.fillStyle = hudSurf.hazard; g.fillRect(0, 0, W, 10); g.fillRect(0, H - 10, W, 10);
  g.fillStyle = hudSurf.cream;
...
.gd{position:fixed;inset:0;pointer-events:none;z-index:44;font:500 13px/1.4 ui-monospace,Menlo,Consolas,monospace;color:var(--p-cream)}
/* specs/0055 5.4 — the guide's own boot cards are DELETED (D16, "boot controls
   A, removing the old menu"). Their whole stylesheet went with the function:
   .gd__cards, .gd__card, .gd__h1, .gd__sub, .gd__credit, .gd__h2, .gd__keys,
   .gd__cap, .gd__what, .gd__go, and body.gd-intro-up with them — nothing sets
   that class any more. intro.js owns the boot flow: one control list, one
   construction, one place to edit it. */
/* specs/0055 §4.1 — GUIDED RUN **A**, TUTORIAL **A** (D6). The prompt becomes a
   sign: a GREEN-CIRCLE PLATE with the key ON it, then the board. A guided run is
   the green circle of §1.8's alphabet — instruction for the easiest run on the
   hill — and saying that in colour lets the tutorial stop apologising for itself
   in words. §2's hard rule "one mark per plate, and a key outranks a symbol" is
   why the key takes the plate and no pictogram is drawn beside it.

   Copy, timing and the stage table are untouched (§4.1). Every colour here is a
   §1.1 token: this file typed 15 literals before 0055 and types none now. */
.gd__toast{position:absolute;left:50%;bottom:16%;transform:translateX(-50%);display:flex;align-items:stretch;gap:0;
  background:rgba(23,22,20,.86);border:0;border-bottom:var(--p-rule) solid var(--p-diff-green);
  border-radius:var(--p-r);overflow:hidden;padding:0;white-space:nowrap;
  box-shadow:0 7px 22px rgba(0,0,0,.38);
  opacity:0;transition:opacity .25s ease}
.gd__toast.is-on{opacity:1}
/* the plate: green circle, and the key sits on it */
.gd__toast b{display:flex;align-items:center;justify-content:center;flex:none;min-width:34px;padding:9px 9px;
  background:var(--p-diff-green);color:var(--p-cream);font-weight:800;text-align:center}
.gd__toast>span{padding:9px 15px}
/* a stage that names no key gets no plate — an empty green block is furniture */
.gd__toast b:empty{display:none}
/* a warning is the same sign with the signal colour on the plate and the rule */
.gd__toast.is-warn{border-bottom-color:var(--p-hazard)}
.gd__toast.is-warn b{background:var(--p-hazard);color:var(--p-ink)}
...
.gd__v.is-hot{color:var(--p-hazard)}
.gd__fin{position:absolute;left:50%;top:38%;transform:translate(-50%,-50%);text-align:center;
  background:rgba(23,22,20,.94);border:1px solid var(--p-hazard);padding:22px 34px;opacity:0;transition:opacity .3s ease}
.gd__fin.is-on{opacity:1}
.gd__fin h3{margin:0 0 4px;font:800 13px/1 ui-monospace,Menlo,monospace;letter-spacing:.2em;color:var(--p-hazard)}
...
/* specs/0055 §4.1 — THE MOUNTING RULE **IS** THE HOLD GAUGE, and the separate
   .gd__bar element is DELETED. The chip already had a 2 px rule along its
   bottom edge; making that rule the progress bar means the one panel in the
   game whose whole job is to show a duration needs no part the system does not
   already have (§1.10: ONE 2 px gauge serves grace, fuel, charge and skip-hold,
   and there is no progress-bar component). --hold is the fill, 0-100 %,
   written by skipTick() where it used to write skipBar.style.width. */
.gd__skip{position:absolute;right:12px;bottom:12px;display:flex;align-items:stretch;gap:0;pointer-events:auto;
  background:rgba(23,22,20,.82);border:0;border-radius:var(--p-r);overflow:hidden;padding:0;cursor:pointer;
  ...
  --hold:0%;
  background-image:linear-gradient(var(--p-cream),var(--p-cream));
  background-repeat:no-repeat;
  background-position:left bottom;
  background-size:var(--hold) var(--p-gauge)}
...
.gd__skip b{display:flex;align-items:center;justify-content:center;flex:none;min-width:22px;padding:5px 7px;
  background:var(--p-cream);color:var(--p-ink);font-weight:800;text-align:center;letter-spacing:0}
.gd__skip>span{padding:5px 11px}
.gd__skip.is-holding{color:var(--p-cream)}
`;
...
function buildDom() {
  ...
  // specs/0055 §5.4 — NO BOOT CARDS. The `.gd__cards` layer, its two
  // `.gd__card` sections and the bootCards() that filled them are deleted.
  // init() was already `.remove()`ing that layer before a frame painted, so
  // what this deletes is an object no player could ever have seen.
  ...
  // specs/0055 §4.1 — `.gd__bar` IS DELETED. The mounting rule carries the hold
  // now (the `--hold` custom property on the chip itself), so the gauge is a
  // property of the sign rather than a fourth child of it.
  skip.append(skipKey, skipTxt);
  skip.hidden = true;

  root.append(toast, race, fin, stageChip, skip);
  document.body.appendChild(root);

  return {
    root, style, toast, tKey, tTxt, race, cells,
    fin, finH, finBig, finRow, finRow2, stageChip, skip, skipTxt, mk,
  };
}
...
export async function init(ctx) {
  try {
    ...
    // ---- THE HOST OWNS THE BOOT FLOW (specs/0003), and as of specs/0055 §5.4
    // it is the ONLY boot flow in the module.
    //
    // TWO BOOT FLOWS WAS THE BUG. bootCards() opened with its own title card and
    ...
    // meant reading two control lists back to back, the second listing keys
    // (MOUSE, SPACE, F) the first deliberately does not.
    //
    // specs/0003 stopped CALLING it; specs/0055 §5.4 (D16 — "boot controls A,
    // removing the old menu") DELETES it, together with `.gd__cards`, both
    // `.gd__card` sections, the ten `.gd__h1`…`.gd__go` rules and the
    // `body.gd-intro-up` switch. Nothing sets that class any more and nothing
    // builds that layer, so there is no empty click trap left to remove here:
    // the object that had to be swept up no longer exists. §10.9 asserts
    // bootCards() is gone; the stage machine below is untouched by any of it.
...
function skipStart() {
  ...
  skipTimer = setInterval(() => {
    ...
    // specs/0055 4.1 — the mounting rule is the gauge; --hold is its fill.
    if (S.dom && S.dom.skip) S.dom.skip.style.setProperty('--hold', (k * 100).toFixed(1) + '%');
...
function skipCancel() {
  ...
  if (S.dom && S.dom.skip) { S.dom.skip.classList.remove('is-holding'); S.dom.skip.style.setProperty('--hold', '0%'); }
}
...
export function skipTutorial() {
  ...
  try {
    ...
    // specs/0055 §5.4 — no `gd-intro-up` to clear: the class, the rule it drove
    // and the cards that set it are all deleted with bootCards().
...
export function update(dt, live) {
  ...
  try {
    ...
    // specs/0061 §1.1 — and the locker's equipped trail, which is the same
    // chevrons on its own prop set and wants the same bob and the same fade
    if (trailProps && trailProps.arrows) fadeArrows(trailProps.arrows, dt);
    if (!S.stage) return;
    S.stageT += dt;
...
// ================================================ specs/0061: the equipped trail
// Greg, 2026-09-06: "I wish there was a little trail to follow based on the run
// you selected (dye + arrows) just like tutorial".
//
// IT IS THE TUTORIAL'S OWN DYE AND CHEVRONS, and that is the whole design.
// `buildDye` and `buildArrows` above already turn a run polyline into a
// ground-conforming stripe and one InstancedMesh of chevrons at GUIDE_CONFIG's
// numbers; a second renderer for the locker's equipped run would be a second
// set of those numbers, and two definitions of what "the guided line" looks
// like is exactly the drift this module's header refuses for the slalom.
//
// WHY IT IS NOT A STAGE. The stage machine owns ONE prop set (`props`) and
// `advanceTo` disposes it on every transition; an equipped trail has to outlive
// stages it has nothing to do with, so it carries a prop set OF ITS OWN and the
// tutorial's teardown never sees it. Nothing above this line changed: the
// stages, their props and their prompts are byte-identical (specs/0061 §1.1).
//
// IT SURVIVES A RESPAWN FOR FREE. The props hang off `S.root`, and `onRespawn`
// re-acquires the route cursor without touching any mesh — the same reason note
// 4 above can promise "the dye is still there".
let trailProps = null;
let trailInfo = null;

// Lay the trail down a run the world declares. `runId` is the contract's own id
// (`world.runs[].id`), or — specs/0061 §1.6 — the LIST of segment ids one run is
// cut into, top segment first: SUNNYSIDE is two rows in the runs export and one
// run on the mountain, and the trail follows the run. `joinRuns` is the same
// splice the tutorial's descent is built with, so a two-segment trail and a
// three-run tutorial leg are one piece of arithmetic.
// specs/0061 (families, 2026-09-06) — A LIST IS NOT ALWAYS A CHAIN. `joinRuns`
// splices ids end to end, which is right for the two halves of SUNNYSIDE and
// wrong for the five lines of the WEST FACE: those all start within 50 m of the
// same shoulder and run PARALLEL to the bottom, so splicing them makes one
// ribbon that skis down the 2nd alternate, teleports back up to the top and
// skis down the 3rd. The list is therefore cut into CHAINS first — a line joins
// the one before it only if it starts where that one ended — and each chain gets
// a dye stripe and a chevron run of its own, in ONE prop set. SUNNYSIDE, DOG LEG
// and CHAMPS ELYSEES come out as one chain, exactly as before; WEST FACE comes
// out as five and POULSEN'S GULLY as four, which is what those trails are.
//
// 8 m in PLAN. The joins this world actually ships are exact to 0.3 m (they are
// interpolated vertices of one clipped OSM way); the nearest thing to a false
// join is POULSEN'S 3's head sitting on POULSEN'S GULLY's head — a HEAD, not a
// tail, so no tolerance reaches it. `joinRuns` already dedups at 1.5 m.
const TRAIL_JOIN_M = 8;
const chainRuns = (ids) => {
  const out = [];
  for (const id of ids) {
    const p = pick(id);
    ...
    if (tail && Math.hypot(tail[0] - p[0][0], tail[2] - p[0][2]) < TRAIL_JOIN_M) {
      cur.ids.push(id);
      cur.pts = joinRuns(cur.ids);
    } else out.push({ ids: [id], pts: p.slice() });
  }
  return out;
};

export function layTrail(runId, { spacing = null, width = null, name = null } = {}) {
  if (!S.ok) return null;
  try {
    const ids = Array.isArray(runId) ? runId.filter((q) => S.runs.has(q)) : null;
    const r = ids && ids.length ? S.runs.get(ids[0]) : S.runs.get(runId);
    const chains = (ids && ids.length ? chainRuns(ids) : (r && r.pts ? [{ ids: [runId], pts: r.pts }] : []))
      .map((c) => ({ ...c, path: c.pts.length >= 4 ? makePath(c.pts, { step: 6 }) : null }))
      .filter((c) => c.path && c.path.L > 0);
    if (!chains.length) return null;
    // EXACTLY ONE TRAIL (§1.3). Equipping another replaces it rather than
    // stacking a second stripe on the hill — and a family's several ribbons are
    // ONE trail: they share `trailProps`, so one `clearTrail()`/one F sweeps up
    // every line of it.
    clearTrail();
    trailProps = newProps();
    const sp = spacing || GUIDE_CONFIG.arrowSpacingM;
    const w = width == null ? GUIDE_CONFIG.dyeWidthM : width;
    let arrows = 0, lengthM = 0;
    for (const c of chains) {
      buildDye(trailProps, c.path, 0, c.path.L, { width: w, step: 5 });
      const A = buildArrows(trailProps, c.path, 0, c.path.L, { spacing: sp });
      // settle the chevrons before the first frame, for the reason init() does it
      // for the teach-W strip: the locker is up, update() is not running the
      // scene, and an unfaded strip is what the player would see on the way out
      if (A) { try { fadeArrows(A, 1); } catch (e) { err(e); } }
      arrows += A ? A.base.length : 0;
      lengthM += c.path.L;
    }
    // THE START IS THE FIRST CHAIN'S HEAD — the caller hands the segments top
    // entrance first (inventory.js sorts them by head height), so this is the
    // top of the trail and `start.yaw` faces down it.
    const first = chains[0].path, last = chains[chains.length - 1].path;
    const h = first.at(0), hd = first.dir(0), t = last.at(last.L);
    trailInfo = {
      id: ids && ids.length ? ids[0] : runId,
      segments: ids && ids.length ? ids.slice() : [runId],
      chains: chains.map((c) => ({ ids: c.ids.slice(), lengthM: +c.path.L.toFixed(1) })),
      name: name || (r && r.name) || (ids && ids[0]) || runId, diff: (r && r.diff) || null,
      lengthM: +lengthM.toFixed(1), spacingM: sp, widthM: w,
      arrows,
      start: { x: +h.x.toFixed(2), y: +h.y.toFixed(2), z: +h.z.toFixed(2), yaw: +hd.yaw.toFixed(4) },
      end: { x: +t.x.toFixed(2), y: +t.y.toFixed(2), z: +t.z.toFixed(2) },
    };
    return trailState();
  } catch (e) { err(e); return null; }
}

// F, away from a lift terminal (§2). Returns how many props it swept up, so the
// acceptance can assert on zero rather than on "it did not throw".
export function clearTrail() {
  const had = trailProps ? trailProps.objs.length : 0;
  if (trailProps) clearProps(trailProps);
  trailInfo = null;
  return had;
}
...
export function trailState() {
  ...
  return { ...trailInfo, props: trailProps ? trailProps.objs.length : 0 };
}
...
export function state() {
  ...
  return {
    ...
    // specs/0061 — the locker's equipped trail, null when nothing is equipped
    trail: trailState(),
...
export const api = {
  ...
  aim,
  config: () => GUIDE_CONFIG,
  // ---- specs/0061: the locker's equipped trail. The locker reaches these
  // through `window.__guide` rather than by importing this module, so the
  // inventory screen gains no dependency on the guided run.
  layTrail,
  clearTrail,
  trailState,
  stageStart,
